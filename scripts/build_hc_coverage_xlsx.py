...
# ⛑ ONE IMPLEMENTATION, NOT A COPY. `num`, `positive_multiple`, `_usable_rate`
# and `derive_valuation` lived HERE and were moved to `providers/valuation.py` on
# 2026-09-08, for the same reason `MINOR_UNITS` moved to `providers/fx_provider`
# the day before: `reporting/generate.py` reads the same vendor and was still
# deriving `Net Debt = EV - marketCap` on mixed units. A copy drifts from the
# original the first time the original changes.
from providers.fx_provider import MINOR_UNITS, major_unit
from providers.valuation import (  # noqa: F401  (re-exported for this module's callers/tests)
    num, positive_multiple, _usable_rate, derive_valuation)  # noqa: E402,F401
