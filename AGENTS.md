...
2. **Update documentation** — refresh `README.md`, `AGENTS.md`, and any other relevant doc files for the changes made during the session. Don't let docs drift behind the code.
...
2. **Anthropic Codex Haiku 4.5** writes a 2-3 line "why" summary from the headlines via `providers/anthropic_summary.py`. The system prompt has a `cache_control` breakpoint so it caches across calls in a single run. Falls back to a headline list if `ANTHROPIC_API_KEY` is missing or the API errors.
...
| `MOVERS_LLM_MODEL`             | `Codex-haiku-4-5`     | Anthropic model for "why" summaries          |
