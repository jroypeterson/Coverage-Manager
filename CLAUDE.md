...
- `scripts/build_hc_coverage_xlsx.py` — **JP's `AA_Core Coverage` workbook**, at
  ...
  **FILENAMES CARRY THE BUILD DATE** — `AA_Core Coverage auto-updated - 08.26.26.xlsx`
  ...
  word is what says at a glance which files a machine owns.

  ⛑ **ARCHIVING MATCHES AN ALLOW-LIST, NEVER AN EXTENSION.** JP: *"I might put my
  own files in this coverage folder for different reasons. Don't move my files. You
  just archive the files you auto-generate."* `archive_previous_autogenerated`
  moves only names in `AUTO_GENERATED_GLOBS`; a parametrised test feeds it the
  near-misses that would break a looser pattern (`AA_Core Coverage NOTES.xlsx`,
  `Jason Peterson Coverage - ENIX.xlsx`, an undated `AA_Core Coverage
  auto-updated.xlsx`). Sweeping by extension would file his work under `archive/`
  with no way to tell that from something he had misplaced himself. Today's output
  is excluded by name, or a same-day rebuild archives what it is about to write.
  ...
  **Columns, and the provenance line.** Order is JP's: `Ticker, Company Name,
  Rating, Mkt Cap, EV, Size, Sector, Subsector, Sub-subsector, Core, Ccy, Price,
  ...
  forward figure with FMP's trailing one under one heading for months, which is why
  a separate `P/E (TTM)` had to be added to the performance report to say what was
  being measured. Verified 2026-08-26: yfinance `enterpriseToRevenue` /
  ...
  **Known limit, stated so the figure does not imply more precision than it has:**
  ...
  means 74% across three years. Published under a heading saying "annual" that
  overstates it roughly threefold. Below -100% yields blank: a security cannot lose
  more than everything and the cube root of a negative growth factor is not real.

  **Precision is one table read by both surfaces** — `DECIMALS` drives the CSV's
  rounding and `NUMFMT` the workbook's number format, with a test asserting they
  agree, or the same figure shows as 22 in the Sheet and 21.94 in the xlsx. Returns,
  Fwd P/E, market cap and EV are whole numbers; Price keeps its cents. Thousands
  separators are a NUMBER FORMAT and never characters in the CSV — writing "650,673"
  would make the Sheet treat the column as text and break every sort on it.

  **Return columns carry a red/white/green colour scale, one per column, white
  pinned to ZERO** rather than the median: a percentile midpoint paints a column
  where everything fell as though half of it were fine. `% of 52W High` is
  deliberately NOT scaled — it runs 0..100 rather than centring on zero. Every surface carries a `LAST UPDATED`
...
- `data/positions_and_researching.csv` — Positions and research list (subset of universe). Replaces `data/watchlist.csv` (deleted 2026-05-03). Schema: `Ticker, Position, Position Date, First Buy Date, Average Cost, Shares, Notes, Held, Held As Of, Previously Held, Held Until`. **`Buy Price` / `Sell Price` were REMOVED 2026-09-05** — price targets now live only on the portfolio workbook's Decision Sheet (`Buy Below` / `Sell Above`), where the reasoning that produced them already sits. Nothing here authored one after 2026-05-03, nothing read them but the weekly report, and the single populated value was being published as a live signal four months after it was set.
  ...
  **A covered broker holding with no positions row gets its row CREATED (board #347, JP 2026-09-26: *"once you see it's in my portfolio shouldn't you just update it automatically"* → *"yes"*).** `plan_sync` lists it in `auto_added`; `apply_plan` creates the row with **no intent flag** — the shape of every held row in the book, since ownership is the `Held` fact and `positions.add` rejects `Portfolio` — stamps provenance in `Notes`, and the ordinary promotion then sets `Held=Y` with the feed's shares and cost. `sync-held` prints one line per ticker (`auto-added <T> - held at <broker>, no coverage row existed`; `--dry-run` prints `would auto-add ...` and writes nothing) and the plan summary carries an `auto-add row` line. Exit 0 — the run did the complete thing. An auto-added name counts as a name joining `Held` for the rename withhold, same as a promotion. Three cases are NOT created: a holding **outside the coverage universe** (`not_in_universe` — needs a sector and a human, reported, exit 2, unchanged); a covered name whose universe row lacks a field `positions.validate` requires (it would flip `positions_status.validation_passed`, which `catalyst_watch` gates the whole export on); and more than `MAX_AUTO_ADDS_PER_RUN` (5) covered rowless holdings in one run (a wrong feed, not a week of purchases — none created). The last two are named in `held_without_row`, exit 2, with the pasteable `positions add` remedy. ⛑ **The demotion circuit breaker now counts WITHHELD demotions too** — an auto-add is a promotion, which withholds demotions, and a breaker counting only the un-withheld list let a feed carrying one new name and none of the 30 real holdings write (Codex, #347 round 2; the same bypass existed before via a promoted or uncovered name).
  ...
  **The export contract did NOT move.** `portfolio.json` is now the rows where `Held == "Y"`, and a held row still publishes `position: "Portfolio"` — see `positions.published_position()`, which is the single rule used by all three export sites (portfolio.json, the sigma-alert payloads, and the joined CSV). No schema bump, no consumer edit; `catalyst_watch` pins `_ACCEPTED_CM_SCHEMA={3,4}` and would hard-fail on an unannounced bump. The four `Held*` columns are APPENDED to the joined CSV — additive, and every consumer reads by name.
  ...
  ⛑ **`SYMBOL_ALIASES` in `held.py` now reads the shared store (board #345 landed 2026-08-27).** It was a hardcoded `{"FISV": "FI"}` pinned at one entry, carrying its own comment saying it was a stopgap and must not be grown. It is now derived from `data/ticker_aliases.json` via `universe.aliases` — see "Symbol aliases" below. Without a join the first sync reports that JP sold Fiserv, because the feed says `FISV` and the universe says `FI`.

  ⛑ **THIS PARAGRAPH USED TO END WITH A CLAIM THAT WAS FALSE, and it is worth keeping the correction visible.** It said an unreadable store "degrades to an EMPTY map, deliberately: an unjoined holding looks like a sale and `MAX_DEMOTIONS_PER_RUN` aborts the run rather than writing one." **That guard fires above FIVE demotions; one unjoined holding is ONE**, so the run wrote the fabricated sale. Three things now stand between the store and a false sale, and the third is the one that actually holds:
  1. an **unreadable** store raises `AliasStoreUnavailable` — nothing is written;
  2. a **missing** store still yields an empty map, because a fleet with no known splits legitimately has none;
  3. **`plan_sync` blocks whenever a demotion coincides with a feed holding that failed to join the universe** — the shape of a fabricated sale, whatever caused it. This is the guard that covers (2) and the case where the store's entry contradicts the universe, neither of which any alias-side check can reach: `held.py` reads `data/ticker_aliases.json` **directly**, so the publish-side guard is irrelevant here. Put the guard at the harm, not at the cause.
  ...
  - `Ready to Buy` — long thesis complete; waiting for the entry trigger (the level itself lives on the workbook's Decision Sheet, as `Buy Below`)
  - `Ready to Short` — short thesis complete; waiting for the entry trigger (`Sell Above` on the Decision Sheet, since short entry is at the high and cover is at the low)
...
- `exports/positions_and_researching.csv` — Positions+researching list joined with the full universe row: every coverage column followed by `Position`, `Position Date`, `First Buy Date`, `Average Cost`, `Shares`, `Notes`. Source `data/positions_and_researching.csv` is the editable source; the join happens at export time. All four `Position` states appear in this CSV.
...
- `exports/watchlist.csv` — **DEPRECATED** back-compat (one cycle): legacy schema (Date Added, Notes — the two price columns left 2026-09-05) derived from positions_and_researching.csv, **filtered to `Portfolio ∪ Researching` only** (preserves the historical contract — `Ready to Buy` / `Ready to Short` rows do not appear here). Use the new state-specific JSON files for new code.
...
- `universe/s1_watch.py` — **weekly S-1 / F-1 IPO-pipeline discovery** (`cli.py s1-watch`, step `[4h/6]`). Added 2026-09-06 after Renaissance Capital's weekly recap named eight registrants and **the build carried five of them, all by hand from that week's Gmail sweep** — a section only as complete as one week's reading is not a lane. Searches S-1/S-1/A/F-1/F-1/A, routes on the registrant's own SIC, and **adds nothing ever**: no price means no market cap means no bucket test, so every row is a watch entry and `auto_add` never sees one. **Six traps, each of which returned a plausible wrong answer first:** (1) **the ticker is NOT a listing test** — registrants reserve a symbol in the S-1, so Syntiant (`SYTN`) and Entrata (`ENT`), the two headline pre-IPO names, both filed as "already trading"; the discriminator is **periodic-reporting history** (a 10-K/10-Q on file), which is a fact about the past a reserved ticker cannot forge. (2) The fee table **moved out of the cover page** with SEC's 2022 fee-tagging rule — read the `EX-FILING FEES` XBRL sidecar's `MaxAggtOfferingPric`, not the S-1 body; reading the cover returned "not stated" for all 104 filings in a window. (3) Those amounts are **mostly placeholders** — SB Energy, Wella, Oura and Accelevation all tag exactly `$100,000,000`; `$1B` is a placeholder too **and is exactly `BUCKET2_CHECK_RAISE`**, so omitting it from `_PLACEHOLDER_VALUES` made the one value that could fire a Bucket 2 flag on no evidence. (4) Dedup keeps the **earliest** filing, so `accession` never changes — comparing it made the size refresh unfireable and the terms-setting S-1/A the one document never read; `latest_accession` is tracked separately. (5) `check_status` needs a **date floor** (`since`): re-filing after a withdrawn 2021-22 registration is the dominant pattern, and any historical `RW` or `424B3` closed the entry, so names flickered in and out. (6) A **sector test cannot find Bucket 2** — it is sector-agnostic at $25B+, so out-of-sector operating registrants (SB Energy, SIC 4911) get their own "Coming public, outside the covered sectors" section rather than the 70-name tail. `partition()` is the single five-way split used by the report, the header and the weekly step, and it **asserts totality** — the body was total while the header summed four of five buckets. Ledger `data/s1_seen.json` carries `reporting_verified` so an unavailable SEC check cannot go green on carry-forward. Corrupt ledger → `LedgerCorrupt`, refuses to overwrite; a failed write → `LedgerNotSaved`, step goes amber. Tests: `tests/test_s1_watch.py` (61).
- `universe/confidential_watch.py` — **the earliest signal in the project, and the only one that depends on the agent looking.** A JOBS Act issuer may submit its S-1 confidentially and need only make it public 15 days before the roadshow, so no lane can see it — but companies announce it 3-6 months ahead. A small JSON ledger (`data/confidential_watch.json`) the weekly agent adds to from its web sweep. **`source_kind` is a strict two-value enum enforced at load** — `announcement` (the company said so; a fact) vs `report` (a journalist said so; a rumour) — rendering in **separate tables**, the second labelled UNCONFIRMED. **The schema has no size field by design**: no filing means no fee table, so any dollar figure came from a journalist. `status` is validated too — it was `row.get(k, "")`, which replaced the `"open"` default with `""`, and both `reconcile()` and `render()` act only on `"open"`, so an entry written without the field disappeared forever. `reconcile()` closes an entry when `s1_watch` sees a matching registrant name, or after 550 days. Tests: `tests/test_confidential_watch.py` (24).
...
sits on the promotion path, not the symbol path. JP decided it on 2026-09-26 (auto-add;
