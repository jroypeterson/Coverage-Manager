...
- `scripts/post_coverage_to_ipo.py` — Posts the weekly additions report to Slack **#ipo-spinoffs-newissues** (`C0BKLC32EHK`; no webhook, so `chat.postMessage` with `SLACK_BOT_TOKEN` + `SLACK_IPO_CHANNEL_ID`). Splits the report: the **lead message** carries the framing, the Recommendations and the pending-approval backlog; the **thread** carries every other section, then one message per company briefing, then a files footer. Sections are routed by heading title and an unrecognised one is threaded, **never dropped** (`test_every_section_is_routed_exactly_once`). Also runs the reversal check below and attaches any finding to the exclusions section. Manual: `python scripts/post_coverage_to_ipo.py --date YYYY-MM-DD [--dry-run] [--preview] [--thread-ts TS] [--channel C…] [--no-reversal-check]`. Replaces hand-rolled payloads in the headless session, which twice delivered the report somewhere unread — see `COVERAGE_APPROVAL_PLAN.md`.
