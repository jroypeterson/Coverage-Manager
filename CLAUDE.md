...
{
  "schema_version": 3,
  "dataset_version": "2026-05-03",
  "generated_at": "2026-05-03T21:08:09Z",
...
**Schema v3 changes (2026-05-03):**
- `Core` column removed from `data/coverage_universe_tickers.csv`. Was selectively populated (only HC Services / MedTech / Other / Life Science Tools); didn't represent a coherent universal "core" subset. The Sector (JP) values already capture the user's mental model of "core = Healthcare Services + MedTech". Removed from `exports/universe.csv` (final column dropped) and `exports/watchlist.json` / `exports/watchlist.csv` (auto-falls-out via the universe-column join). `universe_metadata.json` was never affected — Core wasn't there.
- Downstream consumers reading the documented `universe_metadata.json` contract are unaffected. Anyone reading `universe.csv` / `watchlist.json` / `watchlist.csv` directly should remove references to `"Core"`.
- Report segment renamed: `"Other"` → `"Following: Non-HC"`; HTML suffix `other` → `following_non_hc`. Reason: the prior "Other" tab name collided with the `Sector (JP)` value `"Other"` (46 names), causing confusion in conversation and emails. The new label is unambiguous — it's the slice of the universe being followed that is *not* in Biopharma or HC Svcs/MedTech (Tech + SaaS + Other + Fintech + Life Science Tools, ~144 names). `coverage_other_*.html` archive glob is preserved for back-compat cleanup of pre-rename files.
