...
- **Auto-adds four of the five inclusion buckets** (`universe/auto_add.py`): Bucket 2 (IPO ≥ $25B, any sector), Bucket 3 (spin-off > $10B), **Bucket 1 (core sector — Biopharma/MedTech/Healthcare Services/Life Science Tools — at any size, no floor)** and **Bucket 5 (Russell first-time addition, $2–20B)** enter the universe without being asked. **Bucket 4** ("strategically relevant" $2–20B) and any `New candidate` trigger still queue in `data/candidate_ledger.csv` for a Slack `add TICKER` reply — both are judgement calls by construction. Buckets 1 and 5 were opened up on 2026-08-09 against measured evidence, not preference: replayed over the whole ledger they auto-add 10 names JP approved and neither of the two he declined
