...
- Discovers new candidate tickers via **four** feeds, not one: the Claude discovery prompt (Finnhub IPO calendar + Gmail), a weekly **US exchange symbol-directory diff** (`universe/symbol_directory.py`), a **Form 10-12B watch** for spin-offs and OTC uplistings 1-3 months before they list (`universe/form10_watch.py`), and an **S-1 / F-1 watch** for IPO registrants 3-5 weeks before they price (`universe/s1_watch.py`). A spin-off has no offering, so no IPO calendar can see one; an S-1 filer has no ticker and no market cap, so the calendar, the symbol diff and the Russell lane are all blind to it. Neither lane can ADD anything - no price means no market cap means no bucket test - so both are watch lists feeding the report's pipeline section.
- **Auto-adds three of the five inclusion buckets** (`universe/auto_add.py`): Bucket 2 (IPO ≥ $25B, any sector), Bucket 3 (spin-off > $10B) and **Bucket 1 (core sector — Biopharma/MedTech/Healthcare Services/Life Science Tools — at any size, no floor)** enter the universe without being asked. **Buckets 4 and 5 queue** in `data/candidate_ledger.csv` for a Slack `add TICKER` reply, as does any `New candidate` trigger. **Bucket 5 auto-added from 2026-08-09 and was reversed 2026-09-06** — the ruling rested on the measured claim that the only two names ever declined, `DPC` and `EROC`, "fall in neither bucket", and both turned out to be on the first Russell list under the new rule, both inside $2–20B. Of the four names Bucket 5 did auto-add, **three had been screened out on the merits, in writing, twice each** (`LIME`, `SSMR`, `AADX`). Russell inclusion says a name is institutionally held, not that it belongs here. **A previously declined ticker is now never auto-added under any bucket** — it is re-queued with the decline named. See `plans/bucket5_decision.md`.
...
- **Publishes the weekly additions report as a web page** — <https://jroypeterson.github.io/Coverage-Manager/> (`reporting/weekly_page.py`, `cli.py weekly-page`, refreshed by the `weekly_page` step of `weekly-universe` into `docs/` and served by GitHub Pages). Slack now carries **only the decisions**, as a scannable strip — `Action needed:` on the first line, then a `Decisions` and a `Watch` bullet list (JP 2026-08-17: *"formatted for speed readability first and then be able to delve into details"*). Pipeline, listing-lane findings, exclusions, the per-name reasoning and the full company briefings live on the page, which is bookmarked in `#ipo-spinoffs-newissues` and `#coverage`. Any `###` nested inside a lead section is deferred to the page automatically, so the channel message cannot regrow into an essay
...
| `exports/ticker_aliases.json` | **(NEW 2026-08-27, board #345)** Issuers whose ticker string differs between sources, anchored to the identifiers that did NOT change (CIK / ISIN / composite FIGI). `alias_to_canonical` maps any known symbol to the universe ticker; `vendor_symbols` gives the symbol to send a named vendor. Own `schema_version`. Deliberately silent on which symbol is "current" — no source reliably says. Usually near-empty; consumers do `map.get(sym, sym)` so an absent file degrades to today's behaviour. See `universe/aliases.py`. |
