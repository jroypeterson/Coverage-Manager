...
- **Publishes the weekly additions report as a web page** — <https://jroypeterson.github.io/Coverage-Manager/> (`reporting/weekly_page.py`, `cli.py weekly-page`, refreshed by the `weekly_page` step of `weekly-universe` into `docs/` and served by GitHub Pages). Slack now carries **only the decisions**; pipeline, listing-lane findings, exclusions and the full company briefings live on the page, which is bookmarked in `#ipo-spinoffs-newissues` and `#coverage`
