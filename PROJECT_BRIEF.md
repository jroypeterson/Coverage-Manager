...
| 21 | A covered broker holding with no CM row gets its row created and lands in `Held` | ✅ Done | `acd22e3`/`39a1b6b` (#347) reported it and exited 2; JP decided the policy 2026-09-26 ("once you see it's in my portfolio shouldn't you just update it automatically" → "yes"), so `sync-held` now CREATES the row (no intent flag), promotes it, and names each one (`auto-added <T> - held at <broker>, no coverage row existed`; `--dry-run` shows `would auto-add` and writes nothing). Not created (reported, exit 2): a holding outside the coverage universe; a covered name whose universe row lacks required metadata; or any when more than `MAX_AUTO_ADDS_PER_RUN` (5) would be created in one run (a wrong feed) |
...
- **A held name with no positions row is auto-added** (§21, JP 2026-09-26). The cost,
  stated: CM now asserts a positions row nobody typed. It is limited to names already
  in the coverage universe, capped at 5 per run, and printed per ticker.
